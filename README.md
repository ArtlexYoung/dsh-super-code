...
长程执行树自动整理：每个分支默认保留最近创建的 4 个直接节点，窄侧栏保留 2 个，较深的空闲分支收起下级；运行、诊断/未知、当前会话和焦点路径额外保留。圆圈旁的小箭头手动折叠/展开下级，“自动整理”恢复默认。较早节点进入“已收起”，可搜索、每页 20 条查找并直接打开原始详情；这只是视图归档，不删除会话、不停止成员，待命也不会被标成已完成。已加载目录继续订阅，成员恢复运行会重新显示。用量始终基于完整已加载树；画布自动适应尺寸，手动拖动/缩放后保持用户位置。折叠偏好只保存在当前页面标签内，重载后重新自动整理。
