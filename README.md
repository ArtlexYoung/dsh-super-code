...
长程执行树自动整理：每个分支默认保留最近创建的 4 个直接节点，窄侧栏保留 2 个，较深的空闲分支收起下级；运行、诊断/未知、当前会话和焦点路径额外保留。圆圈旁的小箭头手动折叠/展开下级，“自动整理”恢复默认。较早节点进入“已收起”，在同一列表连续虚拟滚动，仅渲染可视区域及少量缓冲行；支持搜索、创建时间范围（今天、近 7/30 天、自定义日期）及时间、累计 Token、标题双向排序。日期按本地时区且包含结束日；未知时间不参与日期筛选，未知时间或用量排序置后，部分用量以 ≈ 提示。上下方向键、Home/End 可遍历全部匹配项，点击打开原始详情；筛选仅覆盖已加载的收起节点，不额外预读冷目录。这只是视图归档，不删除会话、不停止成员，待命也不会被标成已完成。已加载目录继续订阅，成员恢复运行会重新显示。用量始终基于完整已加载树；画布自动适应尺寸，手动拖动/缩放后保持用户位置。折叠偏好只保存在当前页面标签内，重载后重新自动整理。
