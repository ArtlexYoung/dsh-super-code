...
function lastMatching(messages, predicate) {
...
function compact(messages, maxChars) {
    ...
    const users = [firstUser, ...(lastUser !== undefined && lastUser !== firstUser ? [lastUser] : [])].filter((message) => message !== undefined);
    const required = users.length > 0 ? users : (fallback === undefined ? [] : [fallback]);
    if (required.length === 0)
        return [];
    const truncate = (message, limit) => {
        if (limit <= 0)
            return undefined;
    ...
    const marker = { role: 'tool', content: markerText };
    ...
    if (omitted && remaining >= marker.content.length) {
        ...
        if (value !== undefined)
            result.splice(insertion + 1, 0, value);
    }
    return result;
}
