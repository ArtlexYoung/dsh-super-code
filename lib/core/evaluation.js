...
const buckets = ['uncachedInput', 'cacheRead', 'cacheWrite', 'output'];
...
export const evaluationMeasurementSchema = z.object({
...
function summarize(rows, rates) {
    ...
    const attempts = new Set(), currencies = new Set(), reasons = [];
    const outcomes = {};
    ...
    for (const row of rows) {
        ...
        expectedRequests += expected.size;
        requests += observed.size;
        if (!row.requestManifestComplete)
            reasons.push(`${row.taskId}: incomplete request manifest`);
        if (row.outcome === 'passed' && expected.size === 0)
            reasons.push(`${row.taskId}: passed without model request evidence`);
        for (const id of expected)
            if (!observed.has(id))
                reasons.push(`${row.taskId}: missing request ${id}`);
        for (const request of row.requests) {
            if (!expected.has(request.id))
            ...
            if (!rate || rate.provider !== request.provider || rate.model !== request.model) {
                reasons.push(`${row.taskId}/${request.id}: missing or mismatched price identity`);
                continue;
            }
            currencies.add(rate.currency);
            if (!request.usage || request.toolCharge === undefined) {
                reasons.push(`${row.taskId}/${request.id}: incomplete usage or tool charges`);
                continue;
            }
    ...
    if (!currencies.size && !requests && rates.length)
        for (const rate of rates)
            currencies.add(rate.currency);
    if (currencies.size !== 1)
        reasons.push('No single known currency; no implicit exchange rates');
    if (!Number.isFinite(total) || !Number.isFinite(latencyMs) || buckets.some(key => !Number.isSafeInteger(tokens[key])))
        throw new Error('Evaluation totals exceed numeric capacity');
...
function meetsReduction(baseline, candidate, required) {
...
export function evaluateSuperCodeBatch(input, measurements) {
    ...
    const mismatches = [], missing = [], unmeasured = [];
    ...
    for (const row of rows) {
        ...
        if (group[row.side])
            throw new Error(`Duplicate ${row.side} measurement for ${row.taskId}`);
        group[row.side] = row;
        groups.set(row.taskId, group);
        if (row.category !== expected.category || !same(row.conditions, expected.conditions) || !same(row.identity, manifest[row.side]))
            mismatches.push(`${row.taskId}/${row.side}: comparison identity differs`);
        if (!isScored(row))
            unmeasured.push(`${row.taskId}/${row.side}: ${row.mode}/${row.outcome}`);
        for (const request of row.requests) {
            const identity = JSON.stringify([request.sessionId, request.id]);
            if (requestIdentities.has(identity))
                throw new Error('Duplicate session request across measurements');
    ...
    const pairs = { paired: 0, gained: [], lost: [], unchanged: [], unscored: [] };
    const categories = Object.create(null);
    for (const item of manifest.cases) {
        const group = groups.get(item.taskId);
        for (const side of sides)
            if (!group?.[side])
                missing.push(`${item.taskId}/${side}`);
        const a = group?.baseline, b = group?.candidate;
        if (!a || !b || !isScored(a) || !isScored(b)) {
            pairs.unscored.push(item.taskId);
            continue;
        }
        pairs.paired++;
        const category = categories[item.category] ??= { paired: 0, gained: 0, lost: 0 };
        category.paired++;
        if (a.outcome !== 'passed' && b.outcome === 'passed') {
            pairs.gained.push(item.taskId);
            category.gained++;
        }
        else if (a.outcome === 'passed' && b.outcome !== 'passed') {
            pairs.lost.push(item.taskId);
            category.lost++;
        }
        else
            pairs.unchanged.push(item.taskId);
    }
    for (const [side, summary] of [['baseline', baseline], ['candidate', candidate]]) {
        if (summary.accounting.status === 'unmeasured')
            unmeasured.push(...summary.accounting.reasons.map(reason => `${side}: ${reason}`));
    }
    ...
    if (a.status === 'measured' && b.status === 'measured' && a.currency !== b.currency)
        unmeasured.push('Comparison currencies differ');
