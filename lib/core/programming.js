...
export async function runProgrammingWorkflow(task, callbacks, options = {}, signal = new AbortController().signal) {
    ...
    for (let attempt = 1; attempt <= maxRepairAttempts + 1; attempt += 1) {
        ...
        const feedback = phase === 'repair' ? compactFeedback([finalAcceptance?.repairHint, finalAcceptance?.feedback].filter((value) => typeof value === 'string' && value.trim() !== '').join('\n'), maxFeedbackChars) : undefined;
        ...
        const fingerprint = [acceptance.repairHint, acceptance.feedback].filter((value) => typeof value === 'string' && value.trim() !== '').join('\n').trim();
        if (stopOnRepeatedFeedback && fingerprint !== '' && fingerprint === previousFeedback)
            return { status: 'failed', candidate, attempts: attempt, phases, messages: contextMessages(fingerprint), usage, finalAcceptance };
