...
export async function listToolEvidence(end, before, eventAt, signal) {
    const records = [];
    const page = await scanSessionPage(end, before, eventAt, event => {
        if (event.type === 'tool/call')
            records.push({ seq: event.seq, type: 'call', callId: event.data.callId,
        ...
        if (event.type === 'tool/result' && event.data.message.content.length === 1) {
            ...
            if (block.type === 'tool-result')
                records.push({ seq: event.seq, type: 'result', callId: block.toolCallId,
                    turn: event.data.turn, step: event.data.step, isError: block.isError === true });
        }
        return records.length >= 12;
    }, signal);
    return { kind: 'directory', records, ...page };
}
export function toolEvidenceRef(input) {
    if (!input.sessionId.trim() || !Number.isSafeInteger(input.callSeq) || input.callSeq < 0 || !Number.isSafeInteger(input.resultSeq) || input.resultSeq <= input.callSeq)
        throw new Error('Invalid tool evidence identity');
    return `dsh-tool:${encodeURIComponent(input.sessionId)}:${input.callSeq}:${input.resultSeq}`;
}
export function parseToolEvidenceRef(ref) {
    const match = /^dsh-tool:([^:]+):(0|[1-9]\d*):(0|[1-9]\d*)$/.exec(ref);
    if (!match)
        throw new Error('Expected a dsh-tool evidence reference');
    const result = { sessionId: decodeURIComponent(match[1]), callSeq: Number(match[2]), resultSeq: Number(match[3]) };
    if (toolEvidenceRef(result) !== ref)
        throw new Error('Noncanonical tool evidence reference');
    return result;
}
/** Exact indexed reads only. A genuine tool return is not a test or delivery verdict. */
export function readToolEvidence(sessionId, ref, eventAt, offset) {
    let identity;
    try {
        identity = parseToolEvidenceRef(ref);
    }
    catch {
        return { kind: 'unavailable', reason: 'invalid reference' };
    }
    if (identity.sessionId !== sessionId)
        return { kind: 'unavailable', reason: 'reference belongs to another session' };
    const call = eventAt(identity.callSeq), result = eventAt(identity.resultSeq);
    if (call?.seq !== identity.callSeq || call.type !== 'tool/call' || result?.seq !== identity.resultSeq || result.type !== 'tool/result')
        return { kind: 'unavailable', reason: 'original tool records are missing' };
    const blocks = result.data.message.content;
    if (blocks.length !== 1 || blocks[0]?.type !== 'tool-result' || blocks[0].toolCallId !== call.data.callId || call.data.turn !== result.data.turn || call.data.step !== result.data.step)
        return { kind: 'unavailable', reason: 'tool call and result do not match' };
    ...
    if (offset !== undefined && (!Number.isSafeInteger(offset) || offset < 0 || offset > total))
        return { kind: 'unavailable', reason: 'output offset is outside the recorded text' };
    const slice = (start, end) => {
        ...
        for (let index = 0; index < parts.length && position < end; index++) {
            ...
            if (position + part.length > start)
                value += part.slice(Math.max(0, start - position), Math.max(0, end - position));
            position += part.length;
        }
        return value;
    };
    const isHigh = (text) => /[\uD800-\uDBFF]$/.test(text);
    ...
    if (end < total && isHigh(slice(end - 1, end)))
        end--;
    const excerpt = offset === undefined && total > 8000;
    let output = slice(start, end);
    if (excerpt) {
        let headEnd = 3000, tailStart = total - 4000;
        if (isHigh(slice(headEnd - 1, headEnd)))
            headEnd--;
        if (isHigh(slice(tailStart - 1, tailStart)))
            tailStart++;
    ...
    return { kind: 'recorded', ref, tool: call.data.name, callId: call.data.callId, time: result.time,
        ...
        ...(nextOffset < total ? { readMore: { tool: 'super_code_task', action: 'evidence', record: { ref, offset: nextOffset } } } : {}),
        truncated: excerpt || start > 0 || end < total || call.data.arguments.length > 2000, acceptance: 'not-established', currentCode: 'not-verified' };
}
//# sourceMappingURL=tool-evidence.js.map