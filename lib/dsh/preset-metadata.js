...
export function displayCopy(language) {
...
export async function migrateMetadata(path, id, current, previous, copy, beforeWrite = async () => { }) {
    const baseline = {};
    let original;
    try {
        if (!(await lstat(path)).isFile())
            return { changed: false, baseline: previous ?? {} };
    ...
    for (const field of ['name', 'description']) {
        ...
        if (matches.length !== 1 || !match || /^[>|&*]/.test(match[1].trim()))
            continue;
...
//# sourceMappingURL=preset-metadata.js.map