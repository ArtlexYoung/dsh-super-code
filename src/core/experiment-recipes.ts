...
export const EXPERIMENT_RECIPES = ['reduce', 'differential', 'interleavings', 'metamorphic'] as const
export type ExperimentRecipe = typeof EXPERIMENT_RECIPES[number]

const examples: Readonly<Record<ExperimentRecipe, string>> = {
...
const imports: Readonly<Record<ExperimentRecipe, string>> = {
...
const notes: Readonly<Record<ExperimentRecipe, string>> = {
...
export function readExperimentRecipe(recipe: ExperimentRecipe): string {
  if (!Object.hasOwn(examples, recipe)) throw new Error('Unknown experiment recipe')
