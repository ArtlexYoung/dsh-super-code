...
function metric(value: MetricSnapshot, field: string): MetricSnapshot {
  if (!value || typeof value !== 'object') throw new ProtocolError(`${field} must be an object`, 'INVALID_METRIC')
  if (!['real', 'mock', 'replay'].includes(value.mode)) throw new ProtocolError(`${field}.mode is invalid`, 'INVALID_METRIC')
  for (const key of ['score', 'inputTokens', 'outputTokens', 'latencyMs', 'toolCalls'] as const) {
    if (typeof value[key] !== 'number' || !Number.isFinite(value[key]) || value[key] < 0) throw new ProtocolError(`${field}.${key} must be non-negative and finite`, 'INVALID_METRIC')
  }
  return { ...value }
}
...
export function decideOptimization(
  ...
  candidate: MetricSnapshot | undefined,
  options: {
    readonly changedFactor?: string
    readonly workloadId?: string
    readonly rollbackRef?: string
  ...
  if (base.workloadId !== undefined && next.workloadId !== undefined && base.workloadId !== next.workloadId) throw new ProtocolError('baseline and candidate workloadId must match', 'METRIC_MISMATCH')
  if (options.changedFactor === undefined || options.changedFactor.trim() === '') return { decision: 'inconclusive', notes: ['changedFactor is required to attribute a candidate'] }
  if (options.rollbackRef === undefined || options.rollbackRef.trim() === '') return { decision: 'inconclusive', notes: ['rollbackRef is required for a reversible experiment'] }
  if (base.mode !== 'real' || next.mode !== 'real') return { decision: 'inconclusive', notes: ['real measurements are required for optimization release decisions'] }
...
export class OptimizationLedger {
  ...
  report(maxChars = 4_000): OptimizationReport {
    if (!Number.isSafeInteger(maxChars) || maxChars < 1) throw new ProtocolError('maxChars must be a positive safe integer', 'INVALID_ARGUMENT')
    ...
    const best = ranked[0]
    const next: OptimizationDecision = best === undefined
