...
function bounded(value: string, limit: number): string {
  if (!Number.isSafeInteger(limit) || limit < 1) throw new ProtocolError('maxChars must be a positive safe integer', 'INVALID_ARGUMENT')
...
export function normalizeUrl(raw: string): string {
  ...
  if (url.protocol !== 'http:' && url.protocol !== 'https:') throw new ProtocolError('source URL must use http or https', 'INVALID_URL')
  if (url.username !== '' || url.password !== '') throw new ProtocolError('source URL must not contain credentials', 'INVALID_URL')
  url.hash = ''
...
export function normalizeSource(input: Omit<SourceRecord, 'sourceId'> & { readonly sourceId?: string }): SourceRecord {
  const url = normalizeUrl(input.url)
  const sourceId = input.sourceId === undefined ? sourceIdFor(url) : text(input.sourceId, 'sourceId')
  if (!['primary', 'secondary', 'search'].includes(input.kind)) throw new ProtocolError(`unknown source kind ${String(input.kind)}`, 'INVALID_ARGUMENT')
  return {
    sourceId,
    url,
    kind: input.kind,
...
export class ResearchLedger {
  private readonly sourceMap = new Map<string, SourceRecord>()
  private readonly claimMap = new Map<string, ResearchClaim>()
  /** Add or merge a source by its normalized URL. */
  addSource(input: Omit<SourceRecord, 'sourceId'> & { readonly sourceId?: string }): SourceRecord {
    const source = normalizeSource(input)
    const sameUrl = [...this.sourceMap.values()].find(candidate => candidate.url === source.url)
    if (sameUrl !== undefined) return { ...sameUrl }
    if (this.sourceMap.has(source.sourceId)) throw new ProtocolError(`source ${source.sourceId} already exists with another URL`, 'SOURCE_CONFLICT')
    this.sourceMap.set(source.sourceId, source)
    return { ...source }
  }

  /** Add a claim; source ids are checked before it can be marked verified. */
  addClaim(input: Omit<ResearchClaim, 'counterexamples'> & { readonly counterexamples?: readonly string[] }): ResearchClaim {
    ...
    if (missing.length > 0) throw new ProtocolError(`claim references unknown source(s): ${missing.join(', ')}`, 'SOURCE_NOT_FOUND')
    if (!['verified', 'supported', 'uncertain', 'contradicted'].includes(input.confidence)) throw new ProtocolError(`unknown claim confidence ${String(input.confidence)}`, 'INVALID_ARGUMENT')
    ...
    if (prior !== undefined && stableStringify(prior) !== stableStringify(claim)) throw new ProtocolError(`claim ${claimId} already exists with different content`, 'CLAIM_CONFLICT')
  ...
  addCounterexample(claimId: string, counterexample: string): ResearchClaim {
    ...
    if (claim === undefined) throw new ProtocolError(`unknown claim ${id}`, 'CLAIM_NOT_FOUND')
    const values = [...new Set([...claim.counterexamples, value])]
    const next: ResearchClaim = { ...claim, counterexamples: values, confidence: 'contradicted' }
  ...
  report(maxChars = 4_000): ResearchReport {
    ...
    const status: ResearchReport['status'] = claims.some(claim => claim.confidence === 'contradicted')
      ? 'contradicted'
