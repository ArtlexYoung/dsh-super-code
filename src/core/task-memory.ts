...
export interface TaskMemoryState {
  tasks: Record<string, TaskMemory>; recentSources: number[]
  focus: { kind: 'none' } | { kind: 'task'; id: string }
}
export type TaskMemoryEvent = { kind: 'save'; task: TaskMemory } | { kind: 'focus'; id: string } | { kind: 'archive'; task: TaskMemory }
...
export type TaskMemoryPatch = Partial<Omit<TaskMemory, 'id' | 'createdAtSeq' | 'revision' | 'requirementsRevision' | 'delegationRevision'>> & { removeRequirements?: string[] }
...
export const taskMemorySchema: z.ZodType<TaskMemory> = taskObject

/** A bounded working set. Archived complete records remain in the host log. */
export const taskMemoryStateSchema: z.ZodType<TaskMemoryState> = z.object({
...
export const taskMemoryEventSchema: z.ZodType<TaskMemoryEvent> = z.discriminatedUnion('kind', [
...
export const taskMemoryCreateSchema: z.ZodType<TaskMemoryCreate> = taskObject.omit({ createdAtSeq: true, revision: true, requirementsRevision: true, delegationRevision: true, status: true, evidence: true, decisions: true, next: true })
export const taskMemoryPatchSchema: z.ZodType<TaskMemoryPatch> = z.object({
...
export function emptyTaskMemory(): TaskMemoryState { return { tasks: {}, recentSources: [], focus: { kind: 'none' } } }

export function requireTaskMemory(state: TaskMemoryState, taskId: string): TaskMemory {
  if (!Object.hasOwn(state.tasks, taskId)) throw new Error(`Unknown task ${taskId}; read its archived event to restore it`)
  return state.tasks[taskId]!
}

/** Replace only explicitly named requirements; unchanged constraints retain their sources. */
export function reviseTaskMemory(task: TaskMemory, patch: TaskMemoryPatch, expectedRevision: number): TaskMemory {
  if (task.revision !== expectedRevision) throw new Error(`Stale task ${task.id}: expected ${expectedRevision}, current ${task.revision}`)
  ...
  for (const key of removals) {
    if (!requirements.delete(key)) throw new Error(`Cannot remove unknown requirement ${key}`)
  }
  const replacements = new Set<string>()
  for (const requirement of normalized.requirements ?? []) {
    if (removals.has(requirement.id) || replacements.has(requirement.id)) throw new Error(`Conflicting requirement ${requirement.id}`)
...
export function foldTaskMemory(state: TaskMemoryState, input: TaskMemoryEvent): TaskMemoryState {
...
export function taskMemoryContext(state: TaskMemoryState, maxBytes: number): string {
  ...
  const view = { tasks: roster, recentUserEventSeqs: state.recentSources, ...(focus === undefined ? {} : { current: {
    ...focus,
    evidence: focus.evidence.filter(item => item.requirementsRevision === focus.requirementsRevision && item.sourceVersion === focus.sourceVersion),
  } }) }
  const output = JSON.stringify(view).replace(/\{\{/g, '\\u007b\\u007b')
  if (Buffer.byteLength(output, 'utf8') > maxBytes) throw new Error('Task context exceeds its budget; split the task or explicitly reduce retrievable notes. Hard requirements were not truncated.')
  return output
}
