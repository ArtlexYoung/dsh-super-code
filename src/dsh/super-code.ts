...
import type { TaskMemoryEvent, TaskMemoryState, TaskSource } from '../core/task-memory.js'
...
export interface Config { readonly maxTasks?: number; readonly maxContextBytes?: number }
export const Config: z<Config> = z.object({ maxTasks: z.number().step(1).min(1).max(128).default(32), maxContextBytes: z.number().step(1).min(1024).max(262144).default(32768) })
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const locks = new WeakMap<Session, Promise<unknown>>()
  const stateOf = (session: Session): TaskMemoryState => {
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      const operation = previous.catch(() => {}).then(async (): Promise<string> => {
        exec.signal.throwIfAborted()
        if (pending.has(session)) await checkpoint(session)
        exec.signal.throwIfAborted()
        const state = stateOf(session)
        if (args.action === 'archives') {
          let cursor = args.eventSeq ?? session.seq
          if (!Number.isSafeInteger(cursor) || cursor < 0 || cursor > session.seq) throw new Error('Invalid archive cursor')
        ...
        if (args.action === 'read') return JSON.stringify(requireTaskMemory(state, args.taskId ?? ''))
        ...
        if (args.action === 'restore') {
          ...
          const task = requireTaskMemory(state, args.taskId ?? '')
          if (args.action === 'focus') change = { kind: 'focus', id: task.id }
          else {
        ...
        for (const id of Object.keys(next.tasks)) taskMemoryContext({ ...preview, focus: { kind: 'task', id } }, maxContextBytes)
        exec.signal.throwIfAborted()
      ...
      try { return await operation } finally { if (locks.get(session) === operation) locks.delete(session) }
    },
  }))
}
