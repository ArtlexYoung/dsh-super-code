...
async function main(): Promise<void> {
  ...
  if (!manifestPath || !measurementsPath || extra.length > 0) throw new Error('Usage: super-code-eval manifest.json measurements.jsonl')
  const manifest = JSON.parse(await readFile(manifestPath, 'utf8')) as SuperCodeManifest
  evaluateSuperCodeBatch(manifest, []) // Validate the manifest before reading measurements.
  const rows: SuperCodeMeasurement[] = []
  const input = createReadStream(measurementsPath, { encoding: 'utf8' })
  const lines = createInterface({ input, crlfDelay: Infinity })
  try {
    for await (const line of lines) {
      if (line.trim() === '') continue
      rows.push(JSON.parse(line) as SuperCodeMeasurement)
      if (rows.length > manifest.cases.length * 2) throw new Error('Too many measurements: one baseline and one candidate per case are allowed')
    }
